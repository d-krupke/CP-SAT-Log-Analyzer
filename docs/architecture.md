...
| `insights/` | the colored boxes. `base.py` is the framework (`Trigger`, the `Context` a trigger sees, discovery and the per-trigger error containment), `triggers/*.py` are the two dozen boxes themselves - one class each, threshold and text next to the check that fires them. Four are about the log rather than the solve (not a CP-SAT log, ends before the response, head missing, unrecognized lines) and sort first |
